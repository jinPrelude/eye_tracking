...
def next_batch(i, batch_size):
    os.chdir('/home/leejin/git/image_processing/eye-tracking')
    list = np.loadtxt('dataset/5/eyesPos.csv', dtype='int', delimiter=',')

    start = i * batch_size
    end = start + batch_size

    img_list = cv2.imread('dataset/5/%d.jpg' % start)
...
img = cv2.imread('dataset/5/0.jpg')
...
cost = tf.reduce_mean(tf.square(Y - L10))
